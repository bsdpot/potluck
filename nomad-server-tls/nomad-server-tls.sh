...
if [ -z \${BOOTSTRAP+x} ];
then
    echo 'BOOTSTRAP is unset - please pass in a value of 1, 3, or 5 to set nomad cluster size. Defaulting to 1 if unset. This parameter is optional.'
    BOOTSTRAP=1
fi
...
if [ -z \${NOMADKEY+x} ];
then
    echo 'NOMADKEY is unset - please provide a 32 byte base64 key from the (openssl rand -base64 32) command. Defaulting to same key as GOSSIPKEY if unset.'
    NOMADKEY=\"\$GOSSIPKEY\"
fi
...
# start consul agent configuration #
...
# this is not currently in use, as using cron job to renew certificates and restart services
...
# add the nomad user to the certaccess group
/usr/sbin/pw usermod nomad -G certaccess
...
vault {
  ...
  ca_path = \\\"/mnt/certs/combinedca.pem\\\"
  cert_file = \\\"/mnt/certs/cert.pem\\\"
  key_file = \\\"/mnt/certs/key.pem\\\"
  token = \\\"\$THIS_TOKEN\\\"
