...
# set perms
chmod 750 /root/bin/certrenew.sh

# letsencrypt or zerossl
# check if existing $DOMAIN and create certificates if not
if [ ! -d /mnt/acme/"$DOMAIN"_ecc/ ]; then
    /usr/local/sbin/acme.sh --register-account -m "$EMAIL" --home /mnt/acme --server zerossl
...
# setup system-wide crontab to renew certificate
