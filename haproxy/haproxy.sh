...
if [ -z \${PUBLICIP+x} ]; then
    echo 'PUBLICIP is unset - see documentation to configure this flavour for a public IP address for haproxy frontend. All parameters are mandatory.'
    exit 1
fi
if [ -z \${VAULTSERVER+x} ];
then
...
if [ -s /root/login.token ]; then
    ...
    /usr/local/bin/jq -r '.data.certificate' /mnt/certs/vaultissue.json > /mnt/certs/certkey.pem
    /usr/local/bin/jq -r '.data.private_key' /mnt/certs/vaultissue.json >> /mnt/certs/certkey.pem
    cd /mnt/certs
